...
这里存在一个直接冲突：完整保留历史有利于观察和复现，却会让上下文持续增长；压缩和筛选能够提高上下文效率，却会让模型实际看到的内容不再等于原始轨迹。

mini-SWE-agent 明显优先选择了前者。对范围明确的 benchmark，这是合理的研究取舍。但生产级长任务仍然需要记忆系统、阶段性摘要、上下文压缩与选择机制。这些机制会引入遗漏、错误检索和记忆过期等新风险，却也是长任务无法永久回避的复杂度。

因此，线性历史更适合被理解为“可观测性优先”的研究基线，而不是完整的生产上下文架构。
